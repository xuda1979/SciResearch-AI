...
def main(args):
    ...
    with open(options.dst, "wb") as dst:
        with safe_open(
            os.path.join(options.src, "model.safetensors"), framework="pt", device="cpu"
        ) as src:
            ...
            for token_idx in range(num_text_tokens, num_included_tokens):
                token = o200k_gptoss.decode_single_token_bytes(token_idx).decode(
                    "ascii"
                )
            ...
            assert tokenizer_bytes_written == tokens_size, (
                tokenizer_bytes_written,
                tokens_size,
            )
            write_padding(dst)
            ...
            for n in tqdm(range(num_blocks)):
                ...
                for qkv in (attn_qkv_weight, attn_qkv_bias):
                    ...
                    qk = (
                        qk.view(num_q_heads + num_kv_heads, 2, head_dim // 2, -1)
                        .transpose(1, 2)
                        .reshape(num_q_heads + num_kv_heads, head_dim, -1)
                    )
                    q = qk[:num_q_heads, ...]
                    k = qk[num_q_heads:, ...]
                ...
                write_linear_weight(
                    dst,
                    src.get_tensor(f"block.{n}.attn.out.weight"),
                    src.get_tensor(f"block.{n}.attn.out.bias"),
                )
                ...
                write_linear_weight(
                    dst,
                    src.get_tensor(f"block.{n}.mlp.gate.weight"),
                    src.get_tensor(f"block.{n}.mlp.gate.bias"),
                )
